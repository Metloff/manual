...
Идемпотентные методы - методы, побочные эфекты от N > 0 идентичных запросов которых, идентичных эфекту от одного запроса. Другими словами, если 100 раз послать запрос с методом DELETE на `posts/2` , то мы 100 раз получим одно и то же сообщение в ответ.
